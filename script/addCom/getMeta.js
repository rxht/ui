...
const { readJSONSync, kebabCase } = require('./utils');
const { listFilePath } = require('./consts');
...
module.exports = async () => {
  const meta = await inquirer.prompt([
    {
      type: 'input',
      message: '请输入新组件名（纯英文，大写开头）（Input）：',
      name: 'compName',
      validate(answer) {
        ...
        const listData = readJSONSync(listFilePath);
